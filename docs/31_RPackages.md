...
1. Put any `.csv` or `.xls` data files you have in `data-raw/` sub-directory.  In that same directory, create a R script or Rmarkdown file that reads the data in, cleans it up by renaming columns, or whatever.  An example R script might look something like this:
