...
| `output.var` | What is the name of the `data.frame` object that we want to store the result of the SQL command. If this is missing, the result is just printed to the screen. |
...
```{sql, connection=con, output.var="transactions", echo=FALSE}
/* SQL code Chunk, notice comments are marked in C++ style */
SELECT * FROM Transactions
```
...
```{sql, connection=con, output.var="DereksTransactions"}
